...
CREATE TABLE public.adr_performance_answers (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  assessment_id uuid NOT NULL,
  indicator_code character varying NOT NULL,
  indicator_type character varying NOT NULL CHECK (indicator_type::text = ANY (ARRAY['C'::character varying, 'P'::character varying]::text[])),
  category character varying NOT NULL CHECK (category::text = ANY (ARRAY['A'::character varying, 'B'::character varying, 'C'::character varying, 'D'::character varying, 'E'::character varying]::text[])),
  ...
  CONSTRAINT adr_performance_answers_pkey PRIMARY KEY (id),
  CONSTRAINT adr_performance_answers_assessment_id_fkey FOREIGN KEY (assessment_id) REFERENCES public.adr_performance_assessments(id)
);
CREATE TABLE public.adr_performance_assessments (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  ...
  status character varying DEFAULT 'draft'::character varying CHECK (status::text = ANY (ARRAY['draft'::character varying, 'submitted'::character varying, 'final'::character varying]::text[])),
  notes text,
  created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT adr_performance_assessments_pkey PRIMARY KEY (id),
  CONSTRAINT adr_performance_assessments_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
...
CREATE TABLE public.allergy_card_updates (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  card_id uuid NOT NULL,
  updated_by_name character varying NOT NULL,
  updated_by_organization character varying NOT NULL,
  updated_by_role character varying,
  updated_by_phone character varying,
  updated_by_email character varying,
  facility_name character varying NOT NULL,
  facility_department character varying,
  update_type character varying CHECK (update_type::text = ANY (ARRAY['new_allergy'::character varying, 'medical_facility'::character varying, 'additional_info'::character varying, 'severity_update'::character varying]::text[])),
  update_notes text,
  ...
  CONSTRAINT allergy_card_updates_pkey PRIMARY KEY (id),
  CONSTRAINT allergy_card_updates_card_id_fkey FOREIGN KEY (card_id) REFERENCES public.allergy_cards(id)
);
...
CREATE TABLE public.update_allergies (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT update_allergies_pkey PRIMARY KEY (id),
  CONSTRAINT update_allergies_update_id_fkey FOREIGN KEY (update_id) REFERENCES public.allergy_card_updates(id)
);
