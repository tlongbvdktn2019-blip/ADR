...
export interface ReportPrintTemplateOptions {
  interactive?: boolean
}
...
function escapeHtmlValues<T>(value: T): T {
  if (typeof value === 'string') {
    return escapeHtml(value) as T
  }

  if (Array.isArray(value)) {
    return value.map(item => escapeHtmlValues(item)) as T
  }

  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [key, escapeHtmlValues(item)])
    ) as T
  }
...
export function generateReportPrintHTML(
  sourceReport: ADRReport,
  options: ReportPrintTemplateOptions = {}
): string {
  const report = escapeHtmlValues(sourceReport)
...
<body>
    ${interactive ? `<div class="print-actions no-print">
