...
2. **units** - Khoa/phòng
3. **contests** - Cuộc thi
4. **contest_questions** - Ngân hàng câu hỏi cuộc thi
5. **contest_participants** - Người tham gia
6. **contest_submissions** - Bài thi đã nộp
7. **contest_answers** - Câu trả lời chi tiết
8. **contest_leaderboard** (VIEW) - Bảng xếp hạng
...
contest_questions (1) ---> (*) contest_answers
...
Câu hỏi được lấy từ bảng `contest_questions`. Sử dụng màn hình quản lý câu hỏi cuộc thi hoặc chức năng import Excel để bổ sung ngân hàng câu hỏi.
...
**Giải pháp:**
1. Thêm câu hỏi vào `contest_questions`
